...
| **theme** | Which image/word bank a stream pulls from: `concept` and `reward` (friendly aliases for `Slot::Primary` / `Slot::Alternate`), or `runtime` (`Slot::Runtime`, the last-pulled slot). `theme N` for N≥2 *additional live themes* parses but the compiler rejects it (REQUIRED RUNTIME EXTENSION #1). |
...
themedecl ::= ("concept"|"reward"|"runtime"|"theme" INT) "=" STRING   ; theme N≥2 live: parsed, rejected (Ext #1)
...
theme     ::= "concept" | "reward" | "runtime" | "theme" INT
...
Honest scope: this ships **today at the two-theme floor** (concept = primary, reward =
alternate). Conditioning across **3+ distinct concept-themes at once** is REQUIRED
RUNTIME EXTENSION #1 — the compiler accepts `theme N` syntactically and hard-errors. Two
caveats from this review: (a) pairing **stays locked regardless of phase length** — two
...
| **pair A with B every N** | `Parallel { Action(N, A-effects), Action(N, B-effects) }` — equal period, zero relative offset ⇒ co-fire at position 0 (`ParallelCycler` LCM/reset). Compiler **rejects** non-divisible pairs. Two themes today; 3+ is Ext #1. |
...
| **theme N (N≥2 live)** | parsed; **rejected** — Ext #1. |
...
## 7. Required runtime extensions

Three, each scoped as its own piece of work. None is front-end-only; do not pretend
otherwise.

**Extension #1 — three or more simultaneous live themes (conditioning at scale).**
...
compiler rejects non-divisible pairs so phase-lock cannot silently die). A third live
theme hard-errors (Ext #1).
