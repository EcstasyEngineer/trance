...
{
  if (!path.empty()) {
    static_cast<void>(_font->openFromFile(path));
  }
}
...
std::vector<Font::vertex> Font::get_vertices(const std::string& text, bool large) const
{
  ...
  for (char current : text) {
    ...
    float x1 = pos.x + g.bounds.position.x;
    float y1 = pos.y + g.bounds.position.y;
    float x2 = pos.x + g.bounds.position.x + g.bounds.size.x;
    float y2 = pos.y + g.bounds.position.y + g.bounds.size.y;
    float u1 = float(g.textureRect.position.x) / texture.getSize().x;
    float v1 = float(g.textureRect.position.y) / texture.getSize().y;
    float u2 = float(g.textureRect.position.x + g.textureRect.size.x) / texture.getSize().x;
    float v2 = float(g.textureRect.position.y + g.textureRect.size.y) / texture.getSize().y;
...
Font::rectangle Font::compute_size(const std::string& text, bool large) const
{
  ...
  for (char current : text) {
    ...
    const auto& g = _font->getGlyph(current, char_size(large), false);
    min.x = std::min(min.x, pos.x + g.bounds.position.x);
    max.x = std::max(max.x, pos.x + g.bounds.position.x + g.bounds.size.x);
    min.y = std::min(min.y, pos.y + g.bounds.position.y);
    max.y = std::max(max.y, pos.y + g.bounds.position.y + g.bounds.size.y);
    pos.x += g.advance;
  }
