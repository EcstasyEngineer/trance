...
// auto-created on first run (and auto-migrated from a legacy ./default.session), the
// same bootstrap role default.session played in the original trance.exe.
static const std::string DEFAULT_SESSION_PATH = "default.json";
// Its legacy textproto ancestor: auto-migrated to DEFAULT_SESSION_PATH on a no-arg
// cold start when present (see main.cpp).
static const std::string LEGACY_DEFAULT_SESSION_PATH = "default.session";
