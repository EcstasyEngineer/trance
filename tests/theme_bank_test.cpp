...
namespace
{
  ...
  // The same animation at WIDTH 2 (2x1, 2 frames, 10/100s delays) so a frame's provenance
  // is readable off Image::width() exactly like the PNG fixtures: width 1 = theme "anim",
  // width 2 = theme "anim2". Generated by PIL and embedded verbatim.
  const unsigned char kGif2Frame2x1[] = {
      0x47, 0x49, 0x46, 0x38, 0x39, 0x61, 0x02, 0x00, 0x01, 0x00, 0x81, 0x00, 0x00,
      0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x21,
      ...
      0x2c, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x08, 0x05, 0x00,
      0x01, 0x00, 0x08, 0x08, 0x00, 0x21, 0xf9, 0x04, 0x01, 0x0a, 0x00, 0x01, 0x00,
      0x2c, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x01, 0x00, 0x81, 0xff, 0xff, 0xff,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x05, 0x00, 0x01,
      0x00, 0x08, 0x08, 0x00, 0x3b};
  ...
  // ------------------------------------------------------------------------------------
  // CASE 6 -- A LANE'S ANIMATION FOLLOWS THE LANE'S THEME ACROSS A SWAP.
  //
  // Failure mode (reported live: pinned animation-only theme + slow_flash "stuck on
  // showing something in the UNLOADED slot of the themebank"): each lane's AsyncStreamer
  // preloads its next streamer under whatever theme occupies the lane AT LOAD TIME. When
  // advance_theme() later changes the lane's theme, the force-switch installs that
  // preloaded streamer -- an animation of the OUTGOING theme, which for the primary lane
  // is now slot 0, "unloading". With no change_animation traffic on the lane (slow_flash
  // never sends any for the alternate lane) nothing ever corrects it, so the lane plays
  // the wrong theme's animation until the NEXT swap installs the next stale streamer.
  // For an animation-only theme every `image` draw re-reads the live frame
  // (VisualApiImpl::render_image), so the wrong content is the whole screen.
  //
  // Two animation-only themes tagged by WIDTH (anim = 1x1 gif, anim2 = 2x1 gif). The
  // loop mirrors slow_flash's alternate lane: it never calls change_animation. After
  // each observed lane change a grace window absorbs the async reload; past it, every
  // frame the lane serves must belong to the theme the lane says it holds.
  //
  // get_current_theme_image is held to the strict form of the same rule -- its whole
  // contract is "this lane's CURRENT theme, or nothing" (it feeds the captured-register
  // refresh), so ONE foreign frame from it at ANY time is a failure: a register would be
  // stamped fresh while holding the dead theme's content.
  // ------------------------------------------------------------------------------------
  void test_lane_animation_follows_theme_swap()
  {
    AnimFixture fx;
    fx.root = scratch_root() / "anim_swap";
    std::filesystem::remove_all(fx.root);
    write_file(fx.root / "anim" / "a.gif",
               std::string{reinterpret_cast<const char*>(kGif2Frame), sizeof(kGif2Frame)});
    write_file(fx.root / "anim2" / "b.gif",
               std::string{reinterpret_cast<const char*>(kGif2Frame2x1), sizeof(kGif2Frame2x1)});
    write_file(fx.root / "s.session.json", std::string{R"json({
  "format": "trance-session", "format_version": 1,
  "first_playlist_item": "main",
  "playlist": { "main": { "standard": { "program": "p" } } },
  "program_map": { "p": { "global_fps": 120, "enabled_theme": [
    { "theme_name": "anim", "random_weight": 1 },
    { "theme_name": "anim2", "random_weight": 1 } ] } },
  "theme_scan_root": { "dir": ".", "auto_rescan": false },
  "theme_map": { "anim": { "scan": "anim" }, "anim2": { "scan": "anim2" } }
})json"});
    fx.session =
        load_session_json((fx.root / "s.session.json").string(), fx.root.string(), fx.sidecar);
    fx.system = get_default_system();
    ...
    auto theme_width = [](const std::string& name) -> uint32_t {
      return name == "anim" ? 1u : name == "anim2" ? 2u : 0u;
    };

    // The async pipeline between "the lane's theme changed" and "a fresh streamer for it
    // is buffered and installed" is a handful of async_update passes (evict stale next,
    // load, pump ~8 frames/pass into a 2-frame gif). 200 iterations is more than an
    // order of magnitude of headroom without weakening the assertion: the defect persists
    // for entire inter-swap periods (hundreds to thousands of iterations).
    const int kGrace = 200;

    std::string current_theme = lane_theme();
    int since_change = kGrace;  // the constructor's initial load is synchronous
    std::size_t lane_changes = 0;
    std::size_t sampled = 0;
    std::size_t foreign = 0;
    std::size_t honest_foreign = 0;
    // 32 swap attempts. Whether an attempt changes the PRIMARY lane's theme is the
    // shuffle's business (the incoming slot has to hold the other theme), so the
    // lane_changes >= 2 assertion below is a coin-flip property: at 8 attempts it
    // flaked at ~3.5%; at 32 the miss probability is ~1e-8, comfortably under this
    // file's flake budget.
    for (int i = 0; i < 16000; ++i) {
      fx.bank->async_update();
      fx.bank->advance_frames();
      // Paced, mirroring the engine: VisualApiImpl::change_themes only fires at visual
      // boundaries (hundreds of updates apart). Calling it every iteration would swap
      // the lane constantly and the settled-state window this case is about would never
      // exist. 500 apart keeps 300 settled iterations per period past the grace window.
      if (i % 500 == 499) {
        fx.bank->change_themes();
      }
      ...
      const uint32_t want = theme_width(current_theme);
      // Strict at all times: "current theme or nothing" has no grace period.
      Image strict = fx.bank->get_current_theme_image(false);
      if (strict && strict.width() != want) {
        ++honest_foreign;
      }
      if (since_change < kGrace) {
        continue;
      }
      Image frame = fx.bank->get_animation(false);
      if (frame) {
    ...
    check(lane_changes >= 2, "the primary lane's theme changed at least twice (" +
                                 ...
                                 "case proves nothing");
    check(sampled > 0, "frames were actually sampled outside the grace window (" +
                           std::to_string(sampled) + ")");
    check(foreign == 0,
          "settled well after a swap, a lane serves only its own theme's animation "
          "frames, never the outgoing theme's (foreign: " + std::to_string(foreign) +
              "/" + std::to_string(sampled) + ")");
    check(honest_foreign == 0,
